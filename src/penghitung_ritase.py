"""
Penghitung ritase untuk siklus loading dump truck.

Aturan utama:
- Deteksi berbasis tracker_id, memilih `truck_full` dengan confidence tertinggi.
- Ritase dihitung dari kelas 6 (`truck_full`).
- Siklus global berakhir saat kelas 2 (`bucket_dumping`) muncul.
- Hanya boleh ada satu ritase per siklus global (mencegah duplikasi).
"""
...
# Konstanta ID kelas (sesuaikan dengan label dataset Anda)
CLASS_TRUCK_FULL = 6
CLASS_BUCKET_DUMPING = 2


class FullDeteksi(TypedDict, total=False):
    """Deteksi `truck_full` dalam siklus aktif."""
    full_truck_id: str
    frame_index: int
    confidence: float
...
class KandidatInfo(TypedDict, total=False):
    """Informasi kandidat pada siklus untuk keperluan monitoring/debugging."""
    tracker_id: int
    frame_index: int
    confidence: float
    status: str
...
class PenghitungRitase:
    ...
    - Jika ada beberapa kandidat `truck_full`, pilih yang confidence-nya tertinggi.
    """

    def __init__(self, min_confidence: float = 0.5) -> None:
        """
        Inisialisasi objek penghitung ritase.

        Args:
            min_confidence: Ambang confidence agar deteksi dianggap valid.
        """
        # Penyimpanan data per truck (key = tracker_id)
        self.data_truk: Dict[int, TruckData] = {}

        # Set ID unik `truck_full` yang sudah dihitung
        ...
        # Deteksi per siklus aktif: {tracker_id: [deteksi_truck_full]}
    ...
    def proses_deteksi(
        ...
        """
        Proses satu deteksi untuk penghitungan ritase.

        Args:
            tracker_id: ID tracker truck.
            class_id: 6 untuk `truck_full`, 2 untuk `bucket_dumping`.
            frame_index: Indeks frame saat deteksi terjadi.
            confidence: Skor keyakinan deteksi.

        Returns:
            True jika ada ritase baru pada pemanggilan ini; selain itu False.
        """
        # Daftar truck bila belum ada
        ...
        if class_id == CLASS_TRUCK_FULL:
            ...
            # Jika siklus sudah punya ritase -> hanya evaluasi kandidat terbaik (tidak tambah total)
            if self.siklus_berjalan_punya_ritase:
                ...
                if kandidat_lebih_baik:
                    ...
                    # Kurangi ritase truck lama (jika sebelumnya menjadi kandidat)
                    if kandidat_lama:
                    ...
                    # Tambah ritase pada truck baru sebagai kandidat terbaik
                    ...
                    logger.debug(
                        "Kandidat ritase diperbarui: truck=%s, frame=%s, confidence=%.3f, cycle=%s. "
                        "Confidence lama=%s",
                        tracker_id,
                        frame_index,
                        confidence,
                ...
                # Catat sebagai kandidat (monitoring)
                ...
                logger.debug(
                    "Deteksi ganda dicegah: truck=%s, frame=%s, confidence=%.3f. Total pencegahan=%s",
                    tracker_id,
                    frame_index,
                    confidence,
            ...
            if id_truk_full not in self.truk_full_terhitung and confidence >= self.ambang_kepercayaan:
                # Simpan deteksi pada siklus aktif
                ...
                # Tetapkan sebagai ritase pertama pada siklus ini
                ...
                # Total ritase hanya naik untuk ritase pertama pada siklus
                self.total_ritase += 1
                ...
                logger.debug(
                    "Ritase baru (pertama di siklus): truck=%s, frame=%s, confidence=%.3f, cycle=%s",
                    tracker_id,
                    frame_index,
                    confidence,
                    self.nomor_siklus,
    ...
    def _inisialisasi_truck(self, tracker_id: int) -> None:
        """Buat entri data untuk truck baru."""
    ...
    def selesaikan_siklus(self, tracker_id: int) -> None:
        """
        Tutup siklus aktif untuk truck tertentu dan reset state-nya.

        Args:
            tracker_id: ID truck terkait.
        """
        if tracker_id in self.siklus_aktif:
            # Kosongkan daftar deteksi di siklus aktif truck ini
            self.siklus_aktif[tracker_id] = []

            # Reset best_full agar siap mendeteksi di siklus berikutnya
    ...
    def _selesaikan_siklus_global(self) -> None:
        ...
        Menutup siklus semua truck aktif lalu me-reset state global.
        """
        ...
        if truk_dengan_siklus_aktif or self.siklus_berjalan_punya_ritase:
            ...
            # Catat ringkasan siklus yang berakhir
            if self.siklus_berjalan_punya_ritase and self.ritase_terbaik_di_siklus:
                terbaik = self.ritase_terbaik_di_siklus
                logger.debug(
                    "Siklus global berakhir oleh bucket_dumping. Affected=%s. "
                    "Ritase terbaik: truck=%s, confidence=%.3f, frame=%s. "
                    "Kandidat=%s. Pencegahan ganda=%s.",
            ...
            # Reset state siklus global
            self.siklus_berjalan_punya_ritase = False
            self.ritase_terbaik_di_siklus = None
            self.kandidat_siklus = []
            self.nomor_siklus += 1

            logger.debug("State siklus global direset. Nomor siklus baru=%s", self.nomor_siklus)

    def selesaikan_siklus_manual(self, tracker_id: int, frame_index: int) -> None:
        """
        Tutup siklus secara manual untuk truck tertentu (kasus khusus/fine-tuning).
    ...
    def dapatkan_statistik(self) -> Dict[str, object]:
        """
        Ringkasan statistik penghitungan.

        Returns:
            Dict berisi total ritase, total deteksi, jumlah siklus, dan rincian per truck.
        """
        return {
    ...
    def dapatkan_ritase_count(self, tracker_id: Optional[int] = None) -> int:
        """
        Ambil jumlah ritase untuk truck tertentu atau total semua.

        Args:
            tracker_id: ID truck. Jika None, kembalikan total.
    ...
    def dapatkan_truck_produktif(self) -> List[Dict[str, int | float]]:
        """
        Dapatkan daftar truck yang sudah menghasilkan ritase.

        Returns:
            Daftar dict info truck produktif, diurutkan menurun berdasarkan ritase.
        """
    ...
    def export_summary(self) -> Dict[str, object]:
        """
        Ekspor ringkasan hasil dalam format ringkas untuk laporan.

        Returns:
            Dict: Summary utama, daftar truck produktif, dan detail statistik.
        """
    ...
    def dapatkan_status_siklus_aktif(self) -> Dict[str, object]:
        """
        Status rinci siklus berjalan (untuk debugging/monitoring real-time).

        Returns:
            Dict: Nomor siklus, status ritase, kandidat, dan aktivitas per tracker.
        """
        return {
            "current_cycle_number": self.nomor_siklus,
            "current_cycle_has_ritase": self.siklus_berjalan_punya_ritase,
    ...
    def get_current_cycle_info(self) -> Dict[str, object]:
        ...
        Returns:
            Dict: Status ritase, kandidat terbaik, dan daftar kandidat.
        """
        return {
