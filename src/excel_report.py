"""
Modul pembuatan laporan Excel untuk hasil deteksi excavator/truck.

Fungsi utama:
- Membuat struktur laporan Excel
- Menulis data tracking per ID siklus
- Format kolom: Nama File, ID, Datetime, Ritase, Passing
"""
...
def generate_excel_report(
    ...
    """
    Membuat laporan Excel berisi urutan event (passing/ritase) per ID.

    Kolom:
        - Nama File (hanya pada baris pertama setiap ID)
        - ID (urut berdasar ritase, mulai 1)
        - Datetime (timestamp event)
        - Ritase (akumulasi dalam ID yang sama)
        - Passing (akumulasi dalam ID yang sama)

    Args:
        output_path: Lokasi file Excel keluaran.
        passing_events: Daftar event passing (dict berisi Frame, Detik, Confidence).
        ritase_events: Daftar event ritase (dict berisi Frame, Detik, Confidence).
        video_info: Info video (width, height, fps, total_frames).
        source_path: Path video sumber.
    """
    try:
        wb = openpyxl.Workbook()

        # Gunakan sheet default sebagai "Tracking"
        tracking_sheet = wb.active
        tracking_sheet.title = "Tracking"

        # Header tabel
        headers = ["Nama File", "ID", "Datetime", "Ritase", "Passing"]
        ...
        # Estimasi waktu mulai pemrosesan (agar timestamp event relatif ke proses)
        ...
        # Kumpulkan semua event dalam satu list untuk diurutkan
        all_events = []

        # Tambahkan event passing
        ...
        # Penomoran ID: bertambah setiap kali ada ritase
        current_id = 1
        cycle_counts = {1: {"ritase": 0, "passing": 0}}  # penghitung per ID

        # Tandai setiap event dengan custom_id dan update akumulasi per ID
        for event in all_events:
            event_type = event["type"]
            event["custom_id"] = current_id
        ...
        # Tulis data ke sheet Tracking
        row = 2
        for event in all_events:
            custom_id = event["custom_id"]
            timestamp = event["timestamp"].strftime("%d-%m-%Y %H:%M:%S")

            # Tampilkan nama file hanya sekali pada ID pertama muncul
            filename_value = ""
            if is_first_row_for_id(tracking_sheet, custom_id, row):
                filename_value = filename

            # Hitung akumulasi hingga event saat ini pada ID yang sama
            passing_count = 0
            ritase_count = 0
            for prev_event in all_events:
                if prev_event["timestamp"] > event["timestamp"]:
                    continue
                if prev_event["custom_id"] != custom_id:
                    continue
                if prev_event["type"] == "passing":
                    passing_count += 1
                elif prev_event["type"] == "ritase":
                    ritase_count += 1

            tracking_sheet.cell(row=row, column=1, value=filename_value)
            tracking_sheet.cell(row=row, column=2, value=custom_id)
            tracking_sheet.cell(row=row, column=3, value=timestamp)
        ...
        column_widths = [30, 10, 20, 10, 10]
        for i, width in enumerate(column_widths, 1):
            tracking_sheet.column_dimensions[openpyxl.utils.get_column_letter(i)].width = width
        ...
        info_rows = [
            ...
            ("Durasi (detik)", video_info.total_frames / float(video_info.fps)),
            (
                "Durasi",
                f"{int(video_info.total_frames / float(video_info.fps) // 60)} menit "
                f"{int(video_info.total_frames / float(video_info.fps) % 60)} detik",
            ),
        ...
        # Header detail
        detail_headers = ["No", "Tipe", "ID", "Frame", "Detik", "Timestamp", "Confidence"]
        ...
        for event in all_events:
            ...
            detail_sheet.cell(row=row, column=2, value=event["type"].capitalize())
            detail_sheet.cell(row=row, column=3, value=event["custom_id"])
            detail_sheet.cell(row=row, column=4, value=event["frame"])
            detail_sheet.cell(row=row, column=5, value=event["seconds"])
            detail_sheet.cell(row=row, column=6, value=event["timestamp"].strftime("%d-%m-%Y %H:%M:%S"))
            detail_sheet.cell(row=row, column=7, value=event["confidence"])
            row += 1

        # Lebar kolom Detail Events
        for col in ["A", "B", "C", "D", "E", "F", "G"]:
            detail_sheet.column_dimensions[col].width = 15
        detail_sheet.column_dimensions["F"].width = 20
...
def is_first_row_for_id(sheet, custom_id, current_row):
    """
    Menentukan apakah baris saat ini adalah kemunculan pertama untuk custom_id.

    Args:
        sheet: Worksheet Excel.
        custom_id: ID yang diperiksa.
        current_row: Nomor baris saat ini.

    Returns:
        bool: True jika ini baris pertama untuk custom_id, selain itu False.
    """
    ...
    try:
        ...
        if passing_stats:
            for key, value in passing_stats.items():
                # Hindari data berukuran besar agar sheet tetap ringkas
        ...
        if ritase_stats:
            for key, value in ritase_stats.items():
                # Saring kunci yang tidak perlu ditampilkan
                if key not in ["detail_truck", "current_cycle_has_ritase", "current_cycle_number"]:
                    detail_sheet[f"A{row}"] = key.replace("_", " ").title()
                    detail_sheet[f"B{row}"] = str(value)
                    row += 1

        # Lebar kolom Statistik Detail
        detail_sheet.column_dimensions["A"].width = 30
        detail_sheet.column_dimensions["B"].width = 40

        wb.save(output_path)
        logger.info("Statistik detail berhasil ditambahkan ke laporan.")

    except Exception as exc:
        # Tidak melempar ulang karena laporan dasar sudah ada
